...
/**
 * Reusable SocialPost Component
 * 
 * Features:
 * - Like functionality with counter
 * - Comment system with expandable section
 * - Share button
 * - Support for text, images, and Your posts
 * - Interactive color palette for Your posts
 */
export function SocialPost({ post }: SocialPostProps) {
   ...
   * Toggles like state and updates counter
   */
  const handleLike = () => {
    if (isLiked) {
  ...
  return (
    ...
    >
      ...
      {post.type === 'Your' && post.Your && (
        <div
          className="relative aspect-[16/9] cursor-pointer"
          style={{ background: post.Your }}
          onClick={() => setShowColors(!showColors)}
        >
          <AnimatePresence>
            {showColors && post.colors && (
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
              ...
              >
                <div className="w-full max-w-md">
                  <h3 className="text-white mb-4 text-center">{post.name}</h3>
      ...
      {/* Action Buttons - LIKE & COMMENT FEATURES */}
      ...
      {/* COMMENT SECTION - Expandable comments with reply functionality */}
      <AnimatePresence>
        {showComments && (
          <motion.div
